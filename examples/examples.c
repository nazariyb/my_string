...
void main() {
    ...
    printf("After appending \"%s\": %s\n", test_string0.data, test_string3.data);
    // Example of comparison
    printf("\nComparison\n");
    printf("Comparison \"%s\"function with \"ignite\" returned: %d\n", test_string0.data, my_str_cmp(&test_string0, "ignite"));
    // Example of getting cstr
    const char * cstr = my_str_get_cstr(&test_string3);
    // Example of finding character
    printf("\nFinding character\n");
    printf("Character \'m\' is on position %ld in word \"%s\"", my_str_find_c(&test_string3, '1', 1), test_string3.data);
}